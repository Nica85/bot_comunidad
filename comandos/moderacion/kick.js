...
module.exports = {
    name: 'kick',
    description: 'expulsar a usuario del servidor',
    ...
    options: [
        {
            name: 'usuario',
            description: 'proporciona un usuario a expulsar',
            type: 6,
            required: true,
        },
        {
    ...
    async execute(interaction, client) {
        ...
        await logsadmin.findOne({ serverid: guild.id }).then(async (datos) => {
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(`
            **⚠️ | Realmente quieres expulsar a este usuario?**
            ...
            **Es un bot?:** \`${usuario.user.bot ? 'Si' : 'No'}\`
            ...
            const row = new ActionRowBuilder().addComponents(
                ...
                new ButtonBuilder()
                    ...
                    .setCustomId('kKick-Si')
                    .setLabel('Si')
                    .setEmoji('⚠️'),

                new ButtonBuilder()
                    .setStyle(ButtonStyle.Success)
                    .setCustomId('kKick-No')
                    .setLabel('No')
                    .setEmoji('❓')
            )
            ...
            col.on('collect', async (i) => {
                ...
                switch (i.customId) {
                    ...
                    case 'kKick-Si': {
                        ...
                        if (datos && datos.kick === true) {
                            ...
                            logs.send({
                                embeds: [
                                    new EmbedBuilder()
                                        .setColor(client.color)
                                        .setDescription(`⚠️ | ${usuario.user.tag} fue expulsado del servidor!\n\nRazón: ${razon}`)
                                        .setFooter({ text: `Moderador ${user.tag}`, iconURL: `${user.displayAvatarURL()}` })
                                        .setTimestamp()
                                ]
                            }).catch((error) => { })
                        ...
                        await guild.members.kick(usuario.id, razon).catch((error) => { })

                        interaction.editReply({
                            embeds: [
                                new EmbedBuilder()
                                    .setColor(client.color)
                                    .setDescription(`✅ | ${usuario.user.tag} fue expulsado!\n\nRazón: **${razon}**`)
                            ],
                            components: []
                        })
